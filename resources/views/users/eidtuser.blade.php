...
<div class="container">
  @include('layouts.page_header')
    <div class="form-wrapper card p-4 shadow-sm">
      <h4 class="text-center mb-4">👤 User Update</h4>

      <!-- Image Preview -->
      <img id="preview" class="preview-image" src="{{ asset($users->image) }}" alt="Profile Image Preview" />
      ...
      <form method="POST" action="{{ route('user.update',$users->id) }}" enctype="multipart/form-data">
        ...
        <div class="mb-3">
          ...
          <div class="input-group">
            ...
            <input type="text" class="form-control" value="{{ $users->name }}" placeholder="Enter full name" name="name">
          </div>
          @error('name')
          <p class="mt-2 text-danger small">{{ $message }}</p>
          @enderror
        ...
        <div class="mb-3">
          ...
          <div class="input-group">
            ...
            <input type="tel" class="form-control" value="{{ $users->phone }}" name="phone" placeholder="Enter phone number">
          </div>
          @error('phone')
          <p class="mt-2 text-danger small">{{ $message }}</p>
          @enderror
        ...
        <div class="mb-3">
          ...
          <div class="input-group">
            ...
            <input type="email" class="form-control"value="{{ $users->email }}" name="email" placeholder="Enter email address">
          </div>
          @error('email')
          <p class="mt-2 text-danger small">{{ $message }}</p>
          @enderror
        ...
        <div class="mb-3">
            ...
            <div class="input-group">
              ...
              <input id="password" placeholder="Enter password" value="{{ $users->password }}" type="password" name="password" class="form-control" required autocomplete="new-password">
            </div>
            @error('password')
            <p class="mt-2 text-danger small">{{ $message }}</p>
            @enderror
